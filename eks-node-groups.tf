...
resource "aws_iam_role" "nodes_group" {
  ...
  name = "${var.eks_cluster_name}-nodes_group-role"
...
resource "aws_eks_node_group" "nodes_groups" {
  # Name of the EKS Cluster.
  cluster_name = aws_eks_cluster.eks.name

  # Name of the EKS Node Group.
  node_group_name = "${aws_eks_cluster.eks.name}-nodes_group"
  ...
  subnet_ids = concat("${aws_subnet.main_private[*].id}")
  ...
  labels = {
    role = "${aws_eks_cluster.eks.name}-nodes_group"
  }
